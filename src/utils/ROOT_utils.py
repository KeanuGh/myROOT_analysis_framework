...
def bh_to_TH1(h_bh: bh.Histogram, name: str, title: str, hist_type: str = 'F') -> Type[ROOT.TH1]:
    ...
    h_root = TH1_constructor[hist_type.upper()][n_dims](name, title, *binargs)
...
def TH1_to_bh(h_root: Type[ROOT.TH1]) -> bh.Histogram:
    ...
    # construct axes
    bins = rnp.hist2array(h_root, include_overflow=True, return_edges=True)
    axes = [bh.axis.Variable(ax_bins) for ax_bins in bins[1]]
