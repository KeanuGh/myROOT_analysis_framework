...
class Cutflow:
    ...
    def __init__(self, df: pd.DataFrame,
        ...
        if self.sequential:
            ...
            for cut in cut_dicts:
                curr_cut_columns.append(cut['name'] + cut_label)
                # number of events passing current cut & all previous cuts
                n_events_left = len(df[df[curr_cut_columns].any(1)].index)
            ...
            for cut in cut_dicts:
                # calculations
                n_events_cut = len(df[df[cut['name'] + cut_label]].index)  # number of events passing current cut
                self.cutflow_n_events.append(n_events_cut)
                self.cutflow_ratio.append(len(df[df[cut['name'] + cut_label]].index) / self._n_events_tot)
    ...
    def terminal_printout(self, in_groups: bool = True) -> None:
        """
    ...
    def print_histogram(self, filepath: str, kind: str, in_groups: bool = True, **kwargs) -> None:
        """
