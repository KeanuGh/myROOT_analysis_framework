...
class TestBuildDataframe(object):
    ...
    def test_normal_input(self, tmp_root_datafile):
        output = Dataset._build_dataframe(
            tmp_root_datafile,
    ...
    def test_multifile(self, tmp_root_datafiles):
        expected_output = pd.DataFrame(
            {
                "testvar1": np.concatenate((np.arange(3000), np.arange(2000), np.arange(1000))),
                "testvar3": np.concatenate(
                    (np.arange(3000) * 3, np.arange(2000) * 2, np.arange(1000))
                ),
                "weight_mc": np.concatenate(
                    (
                ...
                "eventNumber": np.concatenate(
                    (np.arange(3000, 6000), np.arange(1000, 3000), np.arange(1000))
                ),
                "totalEventsWeighted": np.concatenate(
                    (
                        np.full(3000, sum(np.append(np.ones(2970), -1 * np.ones(30)))),
        ...
        expected_output = pd.DataFrame(
            {
                ...
                "weight_pileup": np.ones(6000),
                # dataset IDs
                "DSID": np.concatenate((np.full(3000, 3), np.full(2000, 2), np.full(1000, 1))),
    ...
    def test_missing_events_alt_tree(self, tmp_root_datafile_missing_events):
        with pytest.raises(Exception) as e:
            ...
            newmisslist += [newcut]
            _ = Dataset._build_dataframe(
                tmp_root_datafile_missing_events,
                TTree_name=self.default_TTree,
...
class TestCreateCutColumns(object):
    def test_morethan_lessthan(self):
        ...
        out_column1 = pd.Series(
            data=[True, True, True, False, True, True, True, False, True, True],
            name="cut 1" + cut_label,
        )
        out_column2 = pd.Series(
            data=[False, False, True, True, True, True, False, False, False, True],
            name="cut 2" + cut_label,
        )

        assert pd.Series.equals(
            test_df["cut 1" + cut_label], out_column1
        ), f"Expected {out_column1}, got {test_df['cut 1' + cut_label]}"
        assert pd.Series.equals(
            test_df["cut 2" + cut_label], out_column2
    ...
    def test_equals_nequals(self):
        ...
        Dataset._create_cut_columns(test_df, test_cut_dicts)
        out_column1 = pd.Series(
            data=[True, False, True, False, False, False, True, True, True, False],
            name="cut 1" + cut_label,
        )
        out_column2 = pd.Series(
            data=[False, True, False, True, True, True, False, False, False, True],
            name="cut 2" + cut_label,
        )
