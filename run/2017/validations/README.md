...
The most recent threshold-prong diagnostic can be rerun cache-only with:

```bash
pixi run python run/2017/validations/validate_prong_balance_thresholds.py
```

Its corrected cache is:

```text
outputs/validate_shadow_fakes/prong_balance_thresholds/root/validate_prong_balance_thresholds_mtw300.root
```
