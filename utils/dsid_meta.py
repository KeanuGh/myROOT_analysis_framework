...
class DatasetMetadata:
    ...
    def __getitem__(self, id: int) -> DatasetIdMetaContainer:
        self.__err_on_no_dict()
        return self._dsid_meta_dict[id]
    ...
    def fetch_metadata(self, datasets: dict, ttree: str = "T_s1thv_NOMINAL", data_year: int = 2017) -> None:
            ...
            for dataset_name, dataset_dict in datasets.items():
                ...
                if merge_dataset:
                    self.dataset_dsids[dataset_name] = list(dsids)
                self.logger.debug("Found the following dataset IDs for dataset '%s': %s", dataset_name, dsids)
            ...
            for dsid in self._dsid_meta_dict:
                ...
                if not pmg_tool.dsid_exists(dsid):  # for data
                    self._dsid_meta_dict[dsid]["cross_section"] = 1.
                    self._dsid_meta_dict[dsid]["kfactor"] = 1.
                    self._dsid_meta_dict[dsid]["filter_eff"] = 1.
                    ...
                    self._dsid_meta_dict[dsid]["total_size"] = self._convert_size(int(ds_info["totalSize"]))
                    ...
                    try:
                        ...
                        raise ValueError(f"Missing dataset year: {data_year}. Pass one of ['2016', '2017', '2018']") from e
                    ...
                    if not len(ptag_matches):
                        raise ValueError(f"No matching ptags in: {possible_ptags} for dataset: {pattern}")
